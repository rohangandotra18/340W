...
class SelectiveSSM(nn.Module):
    ...
    def _ssm_scan(
        ...
        delta = F.softplus(delta).clamp(max=5.0)  # (B, L, d_inner) Prevent delta explosion
        ...
        for t in range(L):
            ...
            h = h.clamp(-1e3, 1e3)  # Prevent recurrent state explosion causing NaNs
