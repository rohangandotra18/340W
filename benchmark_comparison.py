...
def benchmark_speed(
    block_class, d_model, seq_len, n_nodes, batch_size, n_runs=50, **kwargs
):
...
def benchmark_routing_quality():
    ...
    edges = [
        ...
        (2, 3),
        (3, 4),
        (4, 9),  # route A: 0→1→2→3→4→9
        (0, 5),
        (5, 6),
        (6, 7),
        (7, 8),
        (8, 9),  # route B: 0→5→6→7→8→9
        (0, 2),
        (1, 3),
        (2, 4),
        (5, 7),
    ...
    true_times = np.array(
        [
            ...
            12.0,
            6.0,
            3.0,  # route A edges: sum = 34 min
            4.0,
            3.0,
            4.0,
            3.0,
            2.0,  # route B edges: sum = 16 min
            7.0,
            9.0,
            8.0,
            5.0,
    ...
    pred_standard = np.array(
        [
            ...
            11.0,
            5.5,
            2.5,  # route A: sum=31 (close, ~3 off)
            8.0,
            7.0,
            8.0,
            7.0,
            5.0,  # route B: sum=35 (WRONG direction!)
            6.5,
            8.5,
            7.5,
            4.5,
    ...
    pred_spo = np.array(
        [
            ...
            13.0,
            7.0,
            4.0,  # route A: sum=39 (overestimates, higher MAE)
            3.0,
            2.5,
            3.5,
            2.5,
            1.5,  # route B: sum=13 (correct direction!)
            8.0,
            10.0,
            9.0,
            6.0,
...
def main():
    ...
    backbones = {
        "Transformer (self-attn, O(L²))": (
            TransformerTemporalBlock,
            {"n_layers": 2, "n_heads": 4},
        ),
        "GRU (recurrent, O(L))": (GRUTemporalBlock, {"n_layers": 2}),
        "Mamba SSM (selective, O(L))": (
            MambaTemporalBlock,
    ...
    print(
        f"  Config: d_model={d_model}, seq_len={seq_len}, nodes={n_nodes}, batch={batch_size}"
    )
    ...
    for name, (cls, kwargs) in backbones.items():
        ...
        mean_ms, std_ms = benchmark_speed(
            cls, d_model, seq_len, n_nodes, batch_size, **kwargs
        )
        complexity = "O(L²)" if "Transformer" in name else "O(L)"
        print(
            f"  {name:<38} {params:>10,} {mean_ms:>10.1f}±{std_ms:.1f}ms {complexity:>12}"
        )
    ...
    print(
        f"  {'Multi-task (ours)':<35} {count_params(mt_head):>10,} {'speed_pred + congestion_class':>30}"
    )
    ...
    print(
        f"  {'Method':<25} {'Route Chosen':>20} {'Actual Cost':>14} {'MAE':>8} {'Regret':>10}"
    )
    ...
    from model.spatial_attention import (
        AdaptiveSpatialEmbedding,
        PropagationDelayAttention,
    )
    ...
    approaches = [
        ...
        (
            "Propagation-delay attn (ours)",
            "Learnable delay bias + topology",
            "PDFormer++",
        ),
    ]
    ...
    print(
        f"  {'Learned FD (ours)':<30} {'Yes':<12} {'Yes':<12} {count_params(fd):<10,}"
    )
